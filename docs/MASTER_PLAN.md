...
| R5 saved progress | ✅ Firebase accounts, per-user history, skill-progress checklist, export, delete-my-data |
...
| R9 security/privacy | ✅ rate limit 30/min, non-root container, secrets git-ignored, server-side auth + admin role, enforced e-mail verification |
| R10 test coverage | ✅ 80 pytest + golden eval in CI (`.github/workflows/ci.yml`) |
